...
const ZoneManagement = () => {
  ...
  const [newZone, setNewZone] = useState({ name: '', total: '', location: '' });

  const handleAddZone = () => {
    if (newZone.name && newZone.total) {
      setZones([...zones, { 
        id: zones.length + 1, 
        name: newZone.name, 
        total: parseInt(newZone.total),
        location: newZone.location,
        occupied: 0 
      }]);
      setNewZone({ name: '', total: '', location: '' });
          ...
          <div key={zone.id} className="bg-white p-6 rounded-lg shadow-md">
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                ...
                <span className="font-semibold">{zone.total} slots</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Current Occupancy:</span>
                <span className="font-semibold">{zone.occupied} / {zone.total}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Availability:</span>
                <span className="font-semibold text-green-600">{zone.total - zone.occupied} slots</span>
              </div>
            </div>
          </div>
        ))}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="number"
                  value={newZone.total}
                  onChange={(e) => setNewZone({ ...newZone, total: e.target.value })}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                  placeholder="e.g., 100"
                />
              </div>
